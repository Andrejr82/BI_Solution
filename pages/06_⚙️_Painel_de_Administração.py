...
def get_segmentos_disponiveis():
    ...
    try:
        ...
        # Carregar apenas coluna nomesegmento
        df = pd.read_parquet(parquet_file, columns=['nomesegmento'])
        segmentos = df['nomesegmento'].unique().tolist()
        return segmentos
    except Exception as e:
        st.error(f"Erro ao carregar segmentos: {e}")
    return []
...
if st.session_state.get("authenticated") and st.session_state.get("role") == "admin":
    ...
    with st.expander("➕ Adicionar Novo Usuário"):
        with st.form("add_user_form"):
            ...
            new_role = st.selectbox("Papel", ["user", "admin", "gerente_segmento"])
            
            segmentos = get_segmentos_disponiveis()
            new_segmento = st.selectbox("Segmento", ["Todos"] + sorted(segmentos))
                ...
                if not DB_AVAILABLE:
                    ...
                    if not new_username or not new_password:
                        ...
                        if not is_valid:
                            ...
                            try:
                                segmento_to_db = new_segmento if new_segmento != "Todos" else None
                                auth_db.criar_usuario(sanitized_username, new_password, new_role, new_cloud_enabled, segmento=segmento_to_db)
                                ...
                                audit_logger.info(f"Admin {st.session_state.get('username')} adicionou o usuário {sanitized_username} com papel {new_role}. Cloud: {new_cloud_enabled}. Segmento: {segmento_to_db}")
