...
class SmartLLM:
    ...
    def __init__(self, primary: Optional[str] = None):
        # FIX 2026-01-09: Groq primário por padrão, Gemini desabilitado se API key inválida
    ...
    def gemini(self):
        if self._gemini is None and self._gemini_available:
            try:
                # Import aqui para evitar erro se API key não estiver configurada
                from app.core.llm_gemini_adapter import GeminiLLMAdapter
                self._gemini = GeminiLLMAdapter()
            except Exception as e:
                logger.warning(f"Gemini não disponível (API key expirada ou inválida): {e}")
    ...
    def get_completion(self, messages: List[Dict[str, str]], tools: Optional[Dict] = None) -> Dict[str, Any]:
        ...
        if "error" in result and secondary_adapter:
            ...
            if should_fallback:
                logger.warning(f"🔄 Erro no primário ({self.primary}): {error_str}. Tentando fallback para {secondary_name}...")
                
                fallback_result = secondary_adapter.get_completion(messages, tools)
                
                if "error" not in fallback_result:
                    logger.info(f"✅ Fallback para {secondary_name} funcionou!")
                    return fallback_result
                else:
                    logger.warning(f"❌ Fallback também falhou: {fallback_result.get('error')}")
