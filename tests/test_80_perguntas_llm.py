"""
Script de Teste Completo das 80 Perguntas de Negócio - 100% LLM
Testa cada pergunta usando a LLM e gera relatório detalhado de cobertura
"""
...
# Importar o sistema completo que usa LLM
...
def executar_teste():
    ...
    print("=" * 80)
    print("TESTE COMPLETO DAS 80 PERGUNTAS DE NEGÓCIO - 100% LLM")
    print("=" * 80)
    print(f"Início: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")

    # Inicializar GraphAgent com LLM
    print("Inicializando GraphAgent com LLM...")

    try:
        # Tentar usar Gemini primeiro
        llm = ComponentFactory.get_llm_adapter("gemini")
        agent = GraphAgent(llm=llm)
        print("[OK] GraphAgent inicializado com Gemini\n")
        llm_name = "Gemini"
        ...
        try:
            ...
            agent = GraphAgent(llm=llm)
            print("[OK] GraphAgent inicializado com DeepSeek\n")
            llm_name = "DeepSeek"
    ...
    for categoria, perguntas in PERGUNTAS.items():
        ...
        for idx, pergunta in enumerate(perguntas, 1):
            ...
            try:
                ...
                # Processar com timeout de 60 segundos
                resposta = agent.process(messages)
